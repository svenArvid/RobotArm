...
#include "General.h"
#include "Motor.h"
Motor::Motor(int en_pin, int A1_pin, int A2_pin, int sens_pin, float K_prop, int K_kick_param) :	// class constructor
    ENABLE_PIN(en_pin),	FORWARD_PIN(A1_pin), BACKWARD_PIN(A2_pin), SENSOR_PIN(sens_pin) // use of member initialization list, C++ direkt, p. 232			
{
	...
	K_kick = K_kick_param;		// // MAYBE Set to an in parameter?
	Kp = K_prop;
...
void Motor::Update_position() 
{
	if(dir != 0){
		...
		if (sensor != prev_sensor) 				// positive OR negative edge detected
		{
			pos += dir;
			watchdog_timer = 0;
		}
